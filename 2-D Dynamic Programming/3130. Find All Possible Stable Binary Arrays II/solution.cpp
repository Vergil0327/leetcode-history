#include <vector>
#include <numeric>

using namespace std;

using LL = long long;

class Solution
{
public:
    int numberOfStableArrays(int zero, int one, int limit)
    {
        long long mod = 1e9 + 7;
        vector<vector<vector<long long>>> dp(zero + 1, vector<vector<long long>>(one + 1, vector<long long>(2, 0)));

        for (int i = 1; i <= min(zero, limit); i++)
            dp[i][0][0] = 1;
        for (int j = 1; j <= min(one, limit); j++)
            dp[0][j][1] = 1;

        for (int i = 1; i <= zero; i++)
        {
            for (int j = 1; j <= one; j++)
            {
                // Case 0: current is 0, previous could be 0 or 1
                dp[i][j][0] = (dp[i - 1][j][0] + dp[i - 1][j][1] - (i-1-limit >= 0 ? dp[i-1-limit][j][1] : 0) + mod) % mod;

                // Case 1: current is 1, previous could be 0 or 1
                dp[i][j][1] = (dp[i][j - 1][0] + dp[i][j - 1][1] - (j-1-limit >= 0 ? dp[i][j-1-limit][0] : 0) + mod) % mod;
            }
        }
        return (dp[zero][one][0] + dp[zero][one][1]) % mod;
    }
};