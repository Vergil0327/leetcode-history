...
# Concise Solution, using isalpha()
class Solution:
    def isMatch(self, s: str, p: str) -> bool:
        m, n = len(s), len(p)
        ...
        for i in range(1, m+1):
            for j in range(1, n+1):
                if p[j].isalpha() or p[j] == ".": # just string matching
                    if s[i] == p[j] or p[j] == ".":
                        dp[i][j] = dp[i-1][j-1]

                # change order
                # since problem guarantee pattern is always valid,
                # we don't need to consider out-of-bound index error
                # ! use elif p[j] == "*" not else
                elif p[j] == "*":
                    # choose NOT to use * to match s[i]
                    dp[i][j] = dp[i][j-2]
                    
                    # use * with preceding character to match s[i]
                    dp[i][j] = dp[i][j] or (dp[i-1][j] if s[i] == p[j-1] or p[j-1] == "." else False)
