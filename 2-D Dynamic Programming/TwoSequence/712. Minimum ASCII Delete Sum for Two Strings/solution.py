...
it's simple version of Edit Distance (only delete operation)
"""
# time: O(mn)
# space: O(mn)
class Solution:
...
# we can append dummy character in front of `s1` and `s2` to shift index to 1-based aligned with dp array
class Solution:
    def minimumDeleteSum(self, s1: str, s2: str) -> int:
        m, n = len(s1), len(s2)
        s1 = "#" + s1
        ...
        # dp[i][j]: the minimum delete sum of s1[:i] and s2[:j]
        dp = [[0] * (n+1) for _ in range(m+1)]
