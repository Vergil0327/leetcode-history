...
func Test_dailyTemperaturesBetter(t *testing.T) {
	type args struct {
		temperatures []int
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := dailyTemperaturesBetter(tt.args.temperatures); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("dailyTemperaturesBetter() = %v, want %v", got, tt.want)
			}
		})
	}
}
