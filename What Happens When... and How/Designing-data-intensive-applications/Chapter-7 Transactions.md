...
>
> Historically, durability meant writing to an archive tape. Then it was understood as writing to a disk or SSD. More recently, it has been adapted to mean replication.
...
> - A correlated fault—a power outage or a bug that crashes every node on a particu‐ lar input—can knock out all replicas at once (see “Reliability” on page 6), losing any data that is only in memory. Writing to disk is therefore still relevant for in-memory databases.
...
![Figure 7-2. Violating isolation: one transaction reads another transaction's uncommited writes (a "dirty read")](assets/fig.%207-2.png)
...
![Figure 7-3. Atomicity ensures that if an error occurs any prior writes from that transaction are undone, to avoid an inconsistent state](assets/fig.%207-3.png)
...
In this section we will look at several weak (nonserializable) isolation levels that are used in practice, and discuss in detail what kinds of race conditions can and cannot occur, so that you can decide what level is appropriate to your application. Once we’ve done that, we will discuss serializability in detail (see “Serializability” on page 251). Our discussion of isolation levels will be informal, using examples. If you want rigorous definitions and analyses of their properties, you can find them in the academic literature.
...
![Figure 7-4. No dirty reads: user 2 sees the new value for x only after user1's transaction has committed](assets/fig.%207-4.png)
...
![Figure 7-5. With dirty writes, conflicting writes from different transactions can be mixed up](assets/fig.%207-5.png)
...
![Figure 7-6. Read skew: Alice observes the database in an inconsistent state](assets/fig.%207-6.png)
...
![Figure 7-7. Implementing snapshot isolation using multi-version objects](assets/fig.%207-7.png)
...
1. At the start of each transaction, the database makes a list of all the other transactions that are in progress (not yet committed or aborted) at that time. Any writes that those transactions have made are ignored, even if the transactions subsequently commit.
   - 當transaction開始時, 該時間點正在執行中的所有transactions, 不管是還沒有commit或是已經abort的,  所有這些transaction的寫入都無視
2. Any writes made by aborted transactions are ignored
   - 所有aborted transaction的寫入都無視
3. Any writes made by transactions with a later transaction ID (i.e., which started after the current transaction started) are ignored, regardless of whether those transactions have committed.
   - 所有之後才發生的transactions不管是不是已經commit都無視, 可透過tx ID判斷
4. All other writes are visible to the application's queries
   - 以上條件篩選過後, 剩下的所有其他寫入都是合法的
...
> 換句話說, 在讀取的transaction開始時, 所有在這之前已經commited的created object都是合法的. 並且該object沒有被標示**已刪除**, 即使是被標示**已刪除**, 如果還沒commited的話, 那仍然是合法的
...
> Snapshot isolation: 在Oracle稱作*serializable*; 在PosgreSQL和MySQL稱作*repeatable read*
...
> It is possible, albeit fairly complicated, to express the editing of a text document as a stream of atomic mutations. See “Automatic Conflict Resolution” on page 174 for some pointers.
...
-- Check whether move is valid, then udpate the position of the piece that was returned by the previous SELECT.
...
> 如果content不等於'old content'那就不更新. 但這邊要注意的是, 必須先確定database的實作上不會從舊的snapshot裡讀取並判斷, 不然的話有可能因為old snapshot的關係而導致compare-and-set這操作在lost update的情況下仍然成立
...
> *last write win* (LWW)的方式是很容易造成lost updates的, 但可惜的是LWW是許多replicated database的預設方式
...
![Figure 7-8. Example of write skew causing an application bug](assets/fig.%207-8.png)
...
- The automatic detection of lost updates that you find in some implementations of snapshot isolation unfortunately doesn’t help either: write skew is not auto‐ matically detected in PostgreSQL’s repeatable read, MySQL/InnoDB’s repeatable read, Oracle’s serializable, or SQL Server’s snapshot isolation level. Automatically preventing write skew requires true serializable isolation (see “Serializability” on page 251).
...
- If you can’t use a serializable isolation level, the second-best option in this case is probably to explicitly lock the rows that the transaction depends on. In the doctors example, you could write something like the following:
  - a. As before, **FOR UPDATE** tells the database to lock all rows returned by this query
  ```sql
  BEGIN TRANSACTION;

  SELECT * FROM doctors
    WHERE on_call = true
    AND shift_id = 1234 FOR UPDATE; -- a.
...
> 先query, 然後在更改這些query result可以用**FOR UPDATE**來鎖住目前搜尋的資料. 但如果邏輯是先query存不存在, 不存在的話就寫入. 如果是這樣的話, 由於我們是在query不返回任何資料的情況下寫入, 這情況的write skew並無法用**FOR UPDATE**來解決, 因為我們是沒有資料時才進行寫入的動作, 不像前一種狀況能利用**FOR UPDATE**來鎖住query的資料
...
> 有時single-threaded這類型的執行由於不會有lock的效能耗費, 會比併發還快. 但他的throughput就會受限於單核CPU的運行.
> 而且為了充分利用單一執行緒, transaction也必須用跟傳統不一樣的架構來構造
...
![Figure 7-9. The difference between an interactive transaction and a stored procedure (using the example transaction of Figure 7-8)](assets/fig.%207-9.png)
...
Perhaps not: an algorithm called *serializable snapshot isolation* (SSI) is very promis‐ ing. It provides full serializability, but has only a small performance penalty compared to snapshot isolation. SSI is fairly new: it was first described in 2008 and is the subject of Michael Cahill’s PhD thesis.
...
![Figure 7-10. Detecting when a transaction reads outdated values from an MVCC snapshot](assets/fig.%207-10.png)
...
> transaction必須記住他忽略了其他哪些transaction的寫入. 等到要commit的時候再檢查看有沒有任何一個被忽略的寫入現在變成commited的狀態, 如果有那就必須捨棄(abort)然後重試
...
![Figure 7-11. In serializable snapshot isolation, detecting when one transaction modifies another transaction's reads](assets/fig.%207-11.png)
