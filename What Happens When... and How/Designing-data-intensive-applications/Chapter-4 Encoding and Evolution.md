...
![Figure 4-1. Example record encoded using MessagePack](assets/fig.%204-1.png)
...
![Figure 4-7. When an older version of the application updates data previouly written by a newer version of the application, data may be lost if you're not careful](assets/fig.%204-7.png)
...
- It can act as a buffer if the recipient is unavailable or overloaded, and thus improve system reliability.
  - 當接收端unavailable或overladed時, Message Broker可充當緩衝, 提高reliability
- It can automatically redeliver messages to a process that has crashed, and thus prevent messages from being lost.
  - 當程序掛掉的時候，他可以自動重送訊息，防止message就這麼掉了
- It avoids the sender needing to know the IP address and port number of the recipient (which is particularly useful in a cloud deployment where virtual machines often come and go).
  - sender就不需要知道接收端的IP以及port number, 全交給message broker處理, 這樣在雲端部署的時候方便很多
- It allows one message to be sent to several recipients.
- It logically decouples the sender from the recipient (the sender just publishes messages and doesn’t care who consumes them).
  - 讓sender跟recipient解耦(decouple)

However, a difference compared to RPC is that message-passing communication is usually one-way: a sender normally doesn’t expect to receive a reply to its messages. It is possible for a process to send a response, but this would usually be done on a separate channel. This communication pattern is *asynchronous*: the sender doesn’t wait for the message to be delivered, but simply sends it and then forgets about it.

#### Message brokers

In the past, the landscape of message brokers was dominated by commercial enterprise software from companies such as TIBCO, IBM WebSphere, and webMethods.

More recently, open source implementations such as RabbitMQ, ActiveMQ, HornetQ, NATS, and Apache Kafka have become popular. We will compare them in more detail in Chapter 11.
