...
> replication 最難的部分就在於如何處理資料有所改變這件事.
...
> replication最主要分為三種: single-leader, multi-leader, leaderless
...
![*Figure 5-1. Leader-based (master-slave) replication*](assets/fig.%205-1.png)
...
![*Figure 5-2. Leader-based replication with one synchronous and one asynchronous follower*](assets/fig.%205-2.png)
...
> synchronous 好處, data copy永遠是最新版本，但缺點就是write會因為這同步過程而被block住, 直到follower成功完成replication
...
> 通常只會挑其中一個node作為synchronous node, 不然多個掛掉，整個系統就會因為這些同步而無法繼續正常運行. 有時候被稱為semi-synchronous, 好處是可確保至少有兩個node擁有最新版本的data
...
1. Take a consistent snapshot of the leader’s database at some point in time—if possible, without taking a lock on the entire database. Most databases have this feature, as it is also required for backups. In some cases, third-party tools are needed, such as *innobackupex* for MySQL
   > 持續對database做snapshot

2. Copy the snapshot to the new follower node.
   > 把最新的snapshot複製到新的folloer node上
3. The follower connects to the leader and requests all the data changes that have happened since the snapshot was taken. This requires that the snapshot is associated with an exact position in the leader’s replication log. That position has various names: for example, PostgreSQL calls it the log *sequence number*, and MySQL calls it the *binlog coordinates*.
   > snapshot會有個sequence number註記說後續該從哪個時間點繼續更新資料直到最新版本的leader node, 持續更新
4. When the follower has processed the backlog of data changes since the snapshot, we say it has *caught up*. It can now continue to process data changes from the leader as they happen.
   > 當全部更新完後，我們稱該follower node已經*caught up*, 後續就持續隨著leader node更新資料即可
...
2. *Choosing a new leader.*
   ...
   -  可以透過某個機制選出新的leader node或是由上個選出來的controller node指派，挑選的條件當然是data loss越少越好，所以選資料版本最新的

3. *Reconfiguring the system to use the new leader.*
   - Clients now need to send their write requests to the new leader (we discuss this in “Request Routing”). If the old leader comes back, it might still believe that it is the leader, not realizing that the other replicas have forced it to step down. The system needs to ensure that the old leader becomes a follower and recognizes the new leader.
   - 必須確保現有個follower node的所有write requests都指向新的leader node
   - 並且當舊的leader node恢復時，將他更改設定變為follower node

Failover is fraught with things that can go wrong:

- If asynchronous replication is used, the new leader may not have received all the writes from the old leader before it failed. If the former leader rejoins the cluster after a new leader has been chosen, what should happen to those writes? The new leader may have received conflicting writes in the meantime. The most common solution is for the old leader’s unreplicated writes to simply be discarded, which may violate clients’ durability expectations.
  - 當舊leader node重新回來後, 對於那些沒更新到的write requests, 最常見的做法就是捨棄掉, 因此failover的過程可能會導致使用者對durability的期待
...
- In certain fault scenarios (see Chapter 8), it could happen that two nodes both believe that they are the leader. This situation is called *split brain*, and it is dangerous: if both leaders accept writes, and there is no process for resolving conflicts, data is likely to be lost or corrupted. As a safety catch, some systems have a mechanism to shut down one node if two leaders are detected. However, if this mechanism is not carefully designed, you can end up with both nodes being shut down.
  - mechanism to shut down one node if two leaders are detected: This approach is known as fencing or, more emphatically, Shoot The Other Node In The Head (STONITH). We will discuss fencing in more detail in “The leader and the lock” later
...
- In the case of a log-structured storage engine (see “SSTables and LSM-Trees” on previous chapter), this log is the main place for storage. Log segments are compacted and garbage-collected in the background.
- In the case of a B-tree (see “B-Trees” on previous chapter), which overwrites individual disk blocks, every modification is first written to a write-ahead log so that the index can be restored to a consistent state after a crash.
...
This method of replication is used in PostgreSQL and Oracle, among others. The main disadvantage is that the log describes the data on a very low level: a WAL con‐ tains details of which bytes were changed in which disk blocks. This makes replication closely coupled to the storage engine. If the database changes its storage format from one version to another, it is typically not possible to run different versions of the database software on the leader and the followers.

That may seem like a minor implementation detail, but it can have a big operational impact. If the replication protocol allows the follower to use a newer software version than the leader, you can perform a zero-downtime upgrade of the database software by first upgrading the followers and then performing a failover to make one of the upgraded nodes the new leader. If the replication protocol does not allow this version mismatch, as is often the case with WAL shipping, such upgrades require downtime.
> 如果replication protocol允許follower node的版本比leader node的還高的話，那就可以先升級follower，然後再將leader failover到follower，如此一來即可達到zero downtime
>
> 但如果replication protocol不允許的話，通常就是因為不同版本間storage system的實作細節不相容, 造成WAL shipping必須限制版本, 因而無法允許follower node的版本比leader node高
...
![Figure 5-3. A user makes a write, followed by a read from a stale replica.  To prevent this anomaly, we need read-after-write consistency](assets/fig.%205-3.png)
...
![Figure 5-4. A user first reads from a fresh replica, then from a stale replica. Time appears to go backward. To prevent this anomaly, we need monotonic reads](assets/fig.%205-4.pbg.png)
...
![Figure 5-5. If some partitions are replicated slower than others, an observer may see the answer before they see the question.](assets/Fig.%205-5.png)
...
![Figure 5-6. Multi-leader replication across multiple datacenters](assets/fig.%205-6.png)
...
- *Tolerance of network problems*
  > Traffic between datacenters usually goes over the public internet, which may be less reliable than the local network within a datacenter. A single-leader configuration is very sensitive to problems in this inter-datacenter link, because writes are made synchronously over this link. A multi-leader configuration with asyn‐ chronous replication can usually tolerate network problems better: a temporary network interruption does not prevent writes being processed.
...
> multi-leader replication最大的缺點就是可能同份資料會同時被不同地點給更改，因此需要個解決衝突的機制
...
*Real-time collaborative editing* applications allow several people to edit a document simultaneously. For example, Etherpad and Google Docs allow multiple people to concurrently edit a text document or spreadsheet (the algorithm is briefly discussed in “Automatic Conflict Resolution”).
...
![Figure 5-7. A write conflict caused by two leaders concurrently updating the same record](assets/fig.%205-7.png)
...
![Figure 5-8. Three example topologies in which multi-leader replication can be set up.](assets/fig.%205-8.png)

The most general topology is *all-to-all*(Figure 5-8[c]), in which every leader sends its writes to every other leader. However, more restricted topologies are also used: for example, MySQL by default supports only a *circular topology*, in which each node recevies writes from one node and forwards those writes (plus any writes of its own) to one other node. Another popular topology has the shape of a *star*: one designated root node forwards writes to all of the other nodes. The star topology can be generalized to a tree.
...
![Figure 5-9. With multi-leader replication, writes may arrive in the wrong order at some replicas](assets/FIg.%205-9.png)
...
Some data storage systems take a different approach, abandoning the concept of a leader and allowing any replica to directly accept writes from clients. Some of the earliest replicated data systems were leaderless [1, 44], but the idea was mostly forgotten during the era of dominance of relational databases. It once again became a fashionable architecture for databases after Amazon used it for its in-house Dynamo system. Riak, Cassandra, and Voldemort are open source datastores with leaderless replication models inspired by Dynamo, so this kind of database is also known as *Dynamo-style*.
...
![Figure 5-10. A quorum write, quorum read, and read repair after a node outage](assets/fig.%205-10.png)
...
![FIgure 5-11. If w + r > n, at least one of the r replicas you read from  must have seen the most recent successful write](assets/fig.%205-11.png)
...
- If a sloppy quorum is used (see “Sloppy Quorums and Hinted Handoff” afterwards), the w writes may end up on different nodes than the r reads, so there is no longer a guaranteed overlap between the r nodes and the w nodes.
- If two writes occur concurrently, it is not clear which one happened first. In this case, the only safe solution is to merge the concurrent writes (see “Handling Write Conflicts” on page 171). If a winner is picked based on a timestamp (last write wins), writes can be lost due to clock skew [35]. We will return to this topic in “Detecting Concurrent Writes” afterwards.
...
- If a node carrying a new value fails, and its data is restored from a replica carrying an old value, the number of replicas storing the new value may fall below w, breaking the quorum condition.
  - 原本帶有新value的節點如果掛了, 還原後又變回舊的值. 這時整體的quorum就少了一個，有可能因此低於我們定的`w`. 這可能導致後續讀取到stale data
...
> 如果database有適當的quorums, 那麼就能允許特定數目下的節點即使掛掉, 也可以不用進行failover
> Hinted Handoff: 將臨時轉換/暫存的數據點轉送回其他節點. 例如在網路失聯恢復後, 將臨時暫存的數據點轉送給節點們
...
> [@Jack Lin](https://ithelp.ithome.com.tw/articles/10218182)
>
...
![Figure 5-12 Concurrent writes in a Dynamo-style datastore: there is no well-defined ordering](assets/fig.%205-12.png)
...
LWW achieves the goal of eventual convergence, but at the cost of durability: if there are several concurrent writes to the same key, even if they were all reported as successful to the client (because they were written to w replicas), only one of the writes will survive and the others will be silently discarded. Moreover, LWW may even drop writes that are not concurrent, as we shall discuss in “Timestamps for ordering events” on page.
...
![Figure 5-13. Capturing causal dependencies between two clients concurrently editing a shopping cart](assets/fig.%205-13.png)
...
![Figure 5-14. Graph of causal dependencies in Figure 5-13](assets/fig.%205-14.png)
...
> A *version vector* is sometimes also called a *vector clock*, even though they are not quite the same. The difference is subtle. In brief, when comparing the state of replicas, version vectors are the right data structure to use.
